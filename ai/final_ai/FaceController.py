...
# 이미지 fixel 처리 함수
'''
image : fixel 처리할 crop된 image
factor : fixel 처리할 강도 (default=3.0)
'''
...
# blur, fixel에 대한 수정 옵션 강도 반환 함수
'''
option_type : 'blur', 'fixel'과 같은 수정옵션
...
# 사진 세부 수정 호출 함수
'''
option_type : blur, fixel, swap, sticker
img : 원본 이미지
swap_area : 수정할 사각형 좌표
option_level : blur, fixel의 강도
'''
def face_transform(option_type, img, swap_area, option_level):
    if option_type == 'blur':
        res_img = blur(img, swap_area, option_level)
    elif option_type == 'fixel':
        res_img = fixel(img, swap_area, option_level)
    elif option_type == 'face':
...
# 넘겨받은 얼굴 영역 fixel 처리 요청 함수
'''
img : 원본 image
swap_area : fixel 처리 할 사각형 좌표
option_level : fixel 처리 할 강도
'''
def fixel(img, swap_area, option_level):
    blocks = weights('fixel', option_level)
    if blocks is None:
        return img
