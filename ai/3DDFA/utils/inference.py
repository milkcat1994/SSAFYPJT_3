...
def draw_landmarks(img, pts, style='fancy', wfp=None, show_flg=False, **kwargs):
    ...
    for i in range(len(pts)):
        # 반올림 한 값 저장
        for idx in range(len(pts[i])):
            pts[i][0][idx] = floor(pts[i][0][idx])
            pts[i][1][idx] = floor(pts[i][1][idx])

        # 임시로 얼굴 하나에 대한 좌표를 담기 위한 변수
        landmarks_points = []
        if style == 'simple':
            ...
            for ind in range(len(nums) - 1):
                ...
                for idx in range(l,r):
                    landmarks_points.append((floor(pts[i][0][idx]), floor(pts[i][1][idx])))
